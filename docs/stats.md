...
This column only appears when `shader-minifier-plugin` is checked out as a sibling repo and built locally (`../shader-minifier-plugin`, relative to this repo) — it's not an npm dependency of this project, and the numbers are skipped entirely (row shows `—`) rather than failing the run if it's missing. Each shader shells out to `shader_minifier` via mono (~200ms), which would take minutes across the full corpus — so only the **first 5 packages** (by `tests/package.json` declaration order) get this column; the rest always show `—`. `@babylonjs/core` is first, so it's the one row this column can always rely on for a concrete example.
