...
| `tests/dump-chunks.js`   | Local-only: dumps every shader before/after to gitignored `chunks.dump.txt` for eyeballing |
| `tests/experimental/`    | Local-only pre-release harness (`bun run tests:next`): `*.exp.js` candidates measured + GLSL-gated vs baseline |
...
bun run tests:next  # local-only: run tests/experimental/*.exp.js candidates (not in CI)
