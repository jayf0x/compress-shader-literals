...
| Path                     | Role                                                                                                                                        |
| ------------------------ | ------------------------------------------------------------------------------------------------------------------------------------------- |
| `src/core.js`            | The engine: `extractShaderLiterals` (Babel AST), `extractShaderLiteralsLoose` (regex, `scan: 'loose'`) + `minifyShader`                     |
...
- **Default `include` is the JS/TS family** (`/\.[mc]?[jt]sx?$/` — covers `.mjs/.cjs/.mts/.cts`), because Babel parses whole files. Anything else Babel can't parse as a whole file needs `scan: 'loose'` (regex-based, gated by `SHADER_SIGNAL`) plus a matching `include` — the user points it at the files, this tool makes no claim about which containers work. Note Vite's own SFC compilers often make this moot before it's even needed: `@vitejs/plugin-vue` splits a `.vue` file's `<script>` block into a virtual module whose id already ends in `.ts`/`.js` (e.g. `Foo.vue?vue&type=script&lang.ts`), so `scan: 'ast'` with the default `include` already reaches it — verified with a real Vite+Vue build. Svelte doesn't split like that (whole file stays under the `.svelte` id), so that one does need `scan: 'loose'`.
